  ...
  <div class="default-layout bg-gray-lighter">
    ...
    <ej-app-header v-if="!bodyOnly && hasAuthority"
      ...
      <template #tray>
        ...
        <ej-app-notification v-if="0"/>
        <ej-app-usermenu :user="userData" :endpoint="avatarHeader" :no-uc="true"/>
      </template>

    </ej-app-header>

    <div class="app-body" :class="hasAuthority ? 'pt-md' : ''" v-ej-loading.fullscreen="!loading">
      <nuxt v-if="loading && hasAuthority" class="app-view mx-auto" :style="{width: mainWidth}"/>
      <nuxt v-if="loading && !hasAuthority"/>
      <!-- <nuxt v-if="loading" class="app-view" :class="hasAuthority ? 'mx-auto' : ''"/> -->
    </div>
  </div>
</template>
    ...
    computed: {
      ...
      avatarHeader () {
        return {
          ...
          casLogoutUrl: runtimeArgs.NUXT_ENV_CAS_LOGOUT_URL,/* cas退出地址 */
      ...
      hasAuthority () {
        if (this.loading) {
          // console.log(66, this.userData)待添加跳转大兴地址
          return this.userData.appKeys.includes('DQS')
        }
        return false
      },
    },
